...
class Solution {
    public int[] solution(String[] keyinput, int[] board) {
        ...
        for (String s : keyinput) {
            switch (s) {
                case "left":
                    answer[0] = Math.max(answer[0] - 1, -halfWidth);
                    break;
                case "right":
                    answer[0] = Math.min(answer[0] + 1, halfWidth);
                    break;
                case "up":
                    answer[1] = Math.min(answer[1] + 1, halfHeight);
                    break;
                case "down":
                    answer[1] = Math.max(answer[1] - 1, -halfHeight);
                    break;
            }
        }
...
public class CoordinatesOfCharacter {
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(sol.solution(new String[]{"right", "right", "right", "right","left", "left", "left", "left"}, new int[]{5, 5})));
    }
}
