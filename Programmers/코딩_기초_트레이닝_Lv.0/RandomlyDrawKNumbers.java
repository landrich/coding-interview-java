...
* 문제 설명
  - 랜덤으로 서로 다른 k개의 수를 저장한 배열을 만드려고 합니다. 적절한 방법이 떠오르지 않기 때문에 일정한 범위 내에서
    무작위로 수를 뽑은 후, 지금까지 나온적이 없는 수이면 배열 맨 뒤에 추가하는 방식으로 만들기로 합니다.
...
class Solution {
    public int[] solution(int[] arr, int k) {
        ...
        int[] arrDist = Arrays.stream(arr).distinct().toArray();

        Arrays.fill(answer, -1);
        System.arraycopy(arrDist, 0, answer, 0, Math.min(arrDist.length, k));
